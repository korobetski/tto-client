...
21. **[21-DAILY-QUESTS.md](./21-DAILY-QUESTS.md) - Three quests a day, reset at UTC midnight —
    **built and shipped in `:core` 0.3.0**, and the one document here with no AS3 original: the
    record of why it is shaped this way, written after the fact**
...
| Phase 4: UI Layer | 🔄 IN PROGRESS — 2026-08-12. **The game is playable end to end** and **30 of the 32 screens exist**: character creation with the collection choice, the dashboard and everything behind it (collection browser, deck editor, bag, shop, record with achievements, rules), opponent selection filtered by the hour, deck selection, and the match itself. Also done: the **theme system** (the AS3 palette and Raleway — the plan named the wrong font), **drag-and-drop** alongside tap, and the **turn timer** that `playerPanel` held and that had been missed as a game mechanic. **The `ff8_` collection is reachable for the first time**: the AS3 hard-codes `MODE = 'ff14_'` and never changes it — chosen at creation with a local profile, and on a one-time step after registration with an account, since `POST /accounts` carries no collection. The **avatar picker** (`AvatarScreen`, the original's `AvatarChooser`) is also in: the 27 portraits, opened from the record, written through `ProfileGate` so it serves both flows. **The tutorial and both tournament ladders are done**: all three are scripted matches built as data on the ordinary match screen — the tutorial with the three of its nine lines the original could never reach now restored, the ladders with their thirteen inline opponents extracted to `campaigns.json` and their 500 MGP fee, the only money the game ever takes. **Player versus player shipped 2026-08-12**, which was the last thing blocking a screen: `PVPScreen` is a queue plus invitations and `PVPMatchScreen` renders a `MatchView` the server sends. Of the **2 screens left, neither will be ported** — `EmptyScreen` and `BackstageScreen` are both unreachable in the original and say why. The pre-match animations followed in Phase 6. Nothing is reviewed or approved. See [08-PHASE-4-UI-LAYER.md](./08-PHASE-4-UI-LAYER.md) § What was built | - | - | - |
| Phase 5: Network | 🔄 IN PROGRESS — **the solo-vs-server half is built and running, 2026-08-08.** The legacy socket layer is abandoned (TR-007 re-scoped). The mechanism is a **replayable, signed transcript** the server verifies by re-running the real rules, which is only possible because the Phase 3 engine is pure and deterministic. Delivered: `:core` extracted and published, the transcript and `TranscriptVerifier`, the offline queue, the **version gate** (426 before the body is read), **accounts replacing the local profile** with progression held server-side (aggregates plus per-match history), **several servers** with per-server sessions and queues, five-state connectivity, and **update notices** — a `ClientRelease` announced by the deployment and opened by the app, with no auto-updater and reasons given. Verified end to end against the local Docker/Postgres container. **Re-scoped and then built, 2026-08-12: PvP goes through the server, not peer to peer, and it is in.** That retires the transport question Bluetooth had left open (no common KMP API, and no multiplatform BLE library lets a device advertise — roughly two native implementations) and retires signed moves with it, since the server already attests identity. The cost is named: **two people in one room can no longer play without a connection.** Delivered: `MatchView` and its wire form (the server sends each side only what it may see, and **decides** the playable slots, because Chaos is a roll two devices would disagree about), the queue and invitations, a match row that stores its inputs and its moves so it can be *replayed* rather than believed, the two-minute resume that survives the app being killed, and both screens. The channel is polling — no websocket, as predicted. **Left: the PvP board has no interface test and is tap-only, and nothing has yet exercised the three repositories together.** See [09-PHASE-5-NETWORK.md](./09-PHASE-5-NETWORK.md) § What is left in this phase | - | - | - |
...
| Phase 7: Testing | 🔄 PART DONE — the label was always misleading: unit and UI testing ran far ahead of this phase's schedule (**590 `:shared`, 472 `:core`, 44 server**, all gated in CI with ktlint and detekt). Closed here: the two host `DocumentStore` implementations, which **no test had ever touched** — `androidApp` and `desktopApp` had no test source set at all, and the Android one would not have run in CI even if it had. Genuinely left: **no performance bench** (`EnginePerformanceTest` calls itself a fire alarm against accidental quadratic behaviour, not a bench), and **no instrumented Android or iOS tests**. Blocking issue 4 notes there is **no obtainable AS3 baseline** — AIR is dead — so a bench can only measure absolute targets, never parity. Worth deciding before investing. | - | - | - |
...
compilation. **Phases 4 and 5 are, in substance, done**: every screen the AS3 had that will ever be
ported is ported, and player versus player is playable end to end. What is genuinely left is the
`MODE` removal of document 19 — which documents 18 and 20 are both waiting on — plus the remainder
of the test pass (Phase 7) and three named gaps in the PvP work. Phase 6 is finished.

Outside the migration itself, two things have landed that no phase document anticipates:
**daily quests** ([21-DAILY-QUESTS.md](./21-DAILY-QUESTS.md), shipped in `:core` 0.3.0) and the
**starter pack and formats** of document 19. What remains of 19 is the `MODE` removal, which
is what documents 18 and 20 are waiting on.
