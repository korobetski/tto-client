...
// ---------------------------------------------------------------------------------------
// `clientVersion`, as a constant the running app can read.
//
// `gradle.properties` calls itself "the only place [the release number] is written", and it was
// only half true: `:androidApp` and `:desktopApp` derive their package fields from it, but nothing
// in `commonMain` could see it at all. So the app could not print which build it was, and could
// not compare itself against a published release — the two things the sign-in screen now does.
//
// Deliberately **not** `CURRENT_VERSION`. That one is the *protocol* version, lives in `:core`,
// moves only on a replay-affecting break and is what the version header carries; this one moves on
// every release. `gradle.properties` argues the distinction at length and it is kept here: a build
// numbered 1.0.2 talking protocol 1.0.0 is correct, and collapsing the two would make every patch
// release a compatibility event.
//
// A generated file rather than a `BuildConfig`: there is no such thing in a Kotlin Multiplatform
// library, and the alternatives — a resource to read at runtime, an `expect`/`actual` per host —
// are a file read and three implementations for one string.
// ---------------------------------------------------------------------------------------
val buildVersion = tasks.register("buildVersion") {
    group = "build"
    description = "Writes clientVersion into a Kotlin constant for commonMain."

    val version = providers.gradleProperty("clientVersion")
    val output = layout.buildDirectory.dir("generated/buildVersion")
    // Declared as inputs and outputs rather than read inside the action, so the task is
    // configuration-cache safe and re-runs when — and only when — the number changes.
    inputs.property("clientVersion", version)
    outputs.dir(output)

    doLast {
        val file = output.get().asFile.resolve("com/tripletriad/BuildVersion.kt")
        ...
        file.writeText(
            ...
            /**
             * The release number of this build — `clientVersion` in `gradle.properties`.
             *
             * Generated by `:shared:buildVersion`. Do not edit: every build overwrites it.
             *
             * Not the protocol version. See `com.tripletriad.protocol.CURRENT_VERSION`, which moves
             * on a replay-affecting break and is what a server's gate compares.
             */
            const val CLIENT_VERSION: String = "${version.get()}"
